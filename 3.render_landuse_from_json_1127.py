...
PLANS_DIR  = ROOT_DIR / "result" / "2.plans"
OUT_DIR    = ROOT_DIR / "result" / "3.landuse_flat"
...
def _extract_road_polygons(id_str: str):
    ...
    # 1) 도로 바이너리 (roads_to_parcels 와 같은 기준)
    road_binary = cond_gray < 220           # 어두운 픽셀 = 도로
    road_binary[mask_np < 128] = False      # 대상지 바깥은 전부 road 취급

    kernel = np.ones((3, 3), np.uint8)
    road_dilated = cv2.dilate(road_binary.astype(np.uint8), kernel, iterations=4)

    # 대상지 내부에 있는 도로만 남기기
    road_inside = ((mask_np >= 128) & (road_dilated == 1)).astype(np.uint8) * 255
